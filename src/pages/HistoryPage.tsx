                      ...
                      <AnimatePresence mode="popLayout">
                        {scanHistory.map((record, idx) => (
                          <motion.div
                            ...
                            className={`relative p-5 rounded-3xl border cursor-pointer transition-all duration-300 group ${selectedComparisonIds.includes(record.id)
                          ...
                          >
                            <div className="flex flex-col gap-4">
                              <div className="flex justify-between items-start">
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center relative overflow-hidden ${record.result === 'green' ? 'bg-emerald-500/20' : record.result === 'yellow' ? 'bg-amber-500/20' : 'bg-rose-500/20'}`}>
                                  {record.imageUrl ? (
                                ...
                                <div className="flex gap-2">
                                  ...
                                  <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${selectedComparisonIds.includes(record.id)
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mb-3">
                                  <span className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-md ${record.result === 'green' ? 'bg-emerald-500/30 text-emerald-400' :
                                ...
                                <div className="grid grid-cols-2 gap-2 mt-auto">
                                  <div className="bg-white/5 rounded-xl p-2 border border-white/5">
                                    <p className="text-[10px] text-white/40 uppercase mb-1">{t.safetyScore}</p>
                                    <p className={`font-bold ${record.result === 'green' ? 'text-emerald-400' : record.result === 'yellow' ? 'text-amber-400' : 'text-rose-400'}`}>
                                      {record.safetyScore}%
                                    </p>
                                  </div>
                                  <div className="bg-white/5 rounded-xl p-2 border border-white/5">
                                    <p className="text-[10px] text-white/40 uppercase mb-1">{t.matchScore}</p>
                                    <p className={`font-bold ${record.result === 'green' ? 'text-emerald-400' : record.result === 'yellow' ? 'text-amber-400' : 'text-rose-400'}`}>
                                      {record.matchScore}%
                                    </p>
                                  </div>
